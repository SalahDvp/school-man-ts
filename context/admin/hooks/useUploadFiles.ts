...
type FileObject = {
  ...
  export async function uploadFilesAndLinkToCollection<T>(
    path: string,
    documentId: string,
    files: FileObject[],
  ): Promise<{ name: string; url: string }[]> {
    const collectionRef = collection(db, `${path}`);
    const folderRef = ref(storage, `${path}/${documentId}`);
  ...
  export const updateDocuments = async (
    urlsArray: {name:string;url:string}[],
    ...
    if (deletedFiles.length > 0) {
      deletedFiles.map(async (file) => {
        await updateDoc(doc(db, collectionName, documentId), {
          documents: arrayRemove({ name: file.name, url: file.url }),
        });
        newArray.filter(
  ...
  export async function fetchFiles(urlsArray: { name: string; url: string }[]): Promise<FileObject[]> {
    try {
        ...
        urlsArray
          ...
          .map(async file => {
            try {
              ...
              if (blob) {
                return { file: new File([blob], file.name, { type: blob.type }), source: null, name: file.name };
              } else {
